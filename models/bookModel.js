...
var unissueBook = (book_id, callback) => {
  var sql =
    "UPDATE books SET user_id = NULL, date_issued = NULL WHERE book_id = ?";
...
var getRequestedBooks = (callback) => {
  var sql =
    "SELECT isbn, COUNT(*) AS request_count FROM books_request GROUP BY isbn ORDER BY request_count DESC LIMIT 1";
  db.executeQuery(sql, null, function (result) {
    callback(result);
  });
};
...
var totalBorrowed30 = (callback) => {
  ...
  db.executeQuery(sql, [result, newDate], function (result) {
    callback(result);
  });
};
